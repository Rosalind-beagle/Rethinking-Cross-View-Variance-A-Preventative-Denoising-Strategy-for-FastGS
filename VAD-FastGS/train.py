...
from utils.visualize import compute_variance_and_save_heatmap
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, websockets):
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    # Compute variance, save visualization, and get normalized variance for densification
                    variance_norm = compute_variance_and_save_heatmap(iteration, camlist[-1], gaussians, pipe, bg, render_fastgs, dataset.model_path, args=opt)
            ...
            if iteration % 3000 == 0 and iteration > 15_000 and iteration < 30_000:
                ...
                variance_norm = compute_variance_and_save_heatmap(iteration, camlist[-1], gaussians, pipe, bg, render_fastgs, dataset.model_path, args=opt)
                gaussians.final_prune_fastgs(min_opacity = 0.1, pruning_score = pruning_score, variance_norm = variance_norm)
    ...
    stats = {
        "gaussian_number": gaussians._xyz.shape[0],
        "training_time": total_time
    }
    with open(os.path.join(dataset.model_path, "training_stats.json"), "w") as f:
