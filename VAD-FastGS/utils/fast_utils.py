...
def compute_gaussian_score_fastgs(camlist, gaussians, pipe, bg, args, DENSIFY = False):
    ...
    for view in range(len(camlist)):
        ...
        # ==========================================================
        # PASS 1: The Original FastGS Logic (Indicator Function)
        # ==========================================================
        metric_map_original = (l1_loss_norm > args.loss_thresh).to(torch.float32)
        pkg_original = render_fastgs(my_viewpoint_cam, gaussians, pipe, bg, args.mult, get_flag=get_flag, metric_map=metric_map_original)
        accum_loss_counts_original = pkg_original["accum_metric_counts"]

        # ==========================================================
        # PASS 2: The Modified FastGS Logic (Continuous Error Map)
        # ==========================================================
        metric_map = l1_loss_norm.float()
        ...
        # ==========================================
        with torch.no_grad():
            # Flatten to 1D to avoid broadcasting issues
            scores_flat = accum_loss_counts.flatten() 
            
            # Create a mask for visible Gaussians in this specific view
            valid_mask = (scores_flat > 0)
            
            if valid_mask.any():
                # Get the indices of visible Gaussians
                idx = valid_mask.nonzero(as_tuple=True)[0]
                
                # 1. Update observation counts
                gaussians.view_observe_count[idx] += 1
                new_count = gaussians.view_observe_count[idx]

                # 2. Calculate delta using only the visible subset
                # We use .squeeze() to ensure [M, 1] becomes [M] for calculation
                old_mean = gaussians.error_running_mean[idx].squeeze()
                delta = scores_flat[idx] - old_mean
                
                # 3. Update Running Mean (In-place on the slice)
                # We unsqueeze(1) it back to [M, 1] to match the original shape
                new_mean = old_mean + (delta / new_count.squeeze())
                gaussians.error_running_mean[idx] = new_mean.unsqueeze(1)
                
                # 4. Update Running M2 (In-place on the slice)
                old_m2 = gaussians.error_running_M2[idx].squeeze()
                delta2 = scores_flat[idx] - new_mean
                gaussians.error_running_M2[idx] = (old_m2 + delta * delta2).unsqueeze(1)

        if DENSIFY:
            if full_metric_counts is None:
                full_metric_counts = accum_loss_counts_original.clone()
            else:
                full_metric_counts += accum_loss_counts_original

        if full_metric_score is None:
            full_metric_score = photometric_loss * accum_loss_counts_original.clone()
        else:
            full_metric_score += photometric_loss * accum_loss_counts_original

        # Free up temporary tensors
        del metric_map_original, pkg_original, accum_loss_counts_original
        del render_pkg, accum_loss_counts, scores_flat, metric_map, l1_loss_norm
