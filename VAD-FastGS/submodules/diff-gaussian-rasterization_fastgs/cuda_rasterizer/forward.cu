...
renderCUDA(
	...
	// --- MODIFIED: Change metric arrays from int* to float* to support continuous errors ---
	...
	float* __restrict__ metricCount)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// --- MODIFIED: Continuous Error Accumulation ---
            if(get_flag)
            {
                // Read the continuous error directly from the float metric_map
                float pixel_error = metric_map[pix_id];
                
                // Accumulate the continuous error to the Gaussian's total score.
                // This preserves the peak variance of specular highlights!
                atomicAdd(&(metricCount[collected_id[j]]), pixel_error);
            }
            // -----------------------------------------------
